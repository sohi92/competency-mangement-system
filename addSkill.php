...
// config page.
   ...
   if (isset($_POST['btnSubmit'])) {
		   if($_SERVER["REQUEST_METHOD"] == "POST") {
			...
			$expericace = $_SESSION['expericace'];
			// echo '<h1>'.$Lname.'</h1>';
		...
		// query to insert data employee table.
         $sql = "INSERT INTO employee (Emp_id,Fname, Lname, Contact, Email, Designation, Specialization, Experiance)
		VALUES ('$emp_id','$Fname', '$Lname', '$Contact','$Email','$designation','$special','$expericace')";
		
		if ($db->query($sql) === TRUE && $db->query($sqlset) === TRUE   ) {
		// successfully add in the database.
		$errorMsg = "Details of the employee are added successfully";
